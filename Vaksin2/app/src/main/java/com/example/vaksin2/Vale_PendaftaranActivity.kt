...
import com.example.vaksin2.data.Vale_VaksinDatabase
import com.example.vaksin2.data.Pendaftaran.Vale_AddPendaftaranFragment
import com.example.vaksin2.data.Pendaftaran.Vale_Pendaftaran
import com.example.vaksin2.data.Pendaftaran.Vale_PendaftaranAdapter
import com.example.vaksin2.databinding.ActivityValePendaftaranBinding
import kotlinx.coroutines.launch
import java.io.File

class Vale_PendaftaranActivity : AppCompatActivity() {
    private var _binding : ActivityValePendaftaranBinding? = null
    ...
    lateinit var vaksinDB: Vale_VaksinDatabase

    lateinit var pendaftarList: ArrayList<Vale_Pendaftaran>
    ...
    fun loadDataPendaftar() {
        ...
        lifecycleScope.launch{
            pendaftarList = vaksinDB.getPendaftaranDao().getAllPendaftaran() as ArrayList<Vale_Pendaftaran>
            Log.e("list pendaftar", pendaftarList.toString())
            pendaftaranRecyclerView.adapter = Vale_PendaftaranAdapter(pendaftarList)
        }
    }

    fun deletePendaftar(pendaftar : Vale_Pendaftaran) {
        val builder = AlertDialog.Builder(this@Vale_PendaftaranActivity)
    ...
    fun searchDataPendaftar(keyword : String) {
        ...
        lifecycleScope.launch {
            pendaftarList = vaksinDB.getPendaftaranDao().searchPendaftaran(keyword) as ArrayList<Vale_Pendaftaran>
            Log.e("list pendaftar", pendaftarList.toString())
            pendaftaranRecyclerView.adapter = Vale_PendaftaranAdapter(pendaftarList)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        _binding = ActivityValePendaftaranBinding.inflate(layoutInflater)
        ...
        vaksinDB = Vale_VaksinDatabase(this@Vale_PendaftaranActivity)

        loadDataPendaftar()

        binding.btnAddPendaftar.setOnClickListener {
            Vale_AddPendaftaranFragment().show(supportFragmentManager, "newPendaftarTag")
        }
